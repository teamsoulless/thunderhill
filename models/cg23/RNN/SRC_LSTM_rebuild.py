# ----------------------------------------------------------------------------------
# Self Racing Cars - Stateful RNN Rebuild for Testing - SRC_LSTM_rebuild.py
# ----------------------------------------------------------------------------------

'''
This script rebuilds the Tensorflow graph so that single images can be passed to 
...
(train_seq, valid_seq), (mean, std) = process_csv(filename="data/driving_log_11.csv", val=5)
...
with graph.as_default():
    ...
    # Hyperparameters
    learning_rate = tf.placeholder_with_default(input=1e-7, shape=())
...
with tf.Session(graph=graph) as session:#, config=tf.ConfigProto(gpu_options=gpu_options)) as session:
