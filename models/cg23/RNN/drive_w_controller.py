...
def stanley(cte, speed):
    K = 1.0 # Stanley gain
    steering = np.arctan((K*abs(cte))/speed)
    if cte > 0:
        steering = -steering
    return steering
...
def telemetry(sid, data):
    ...
    # Now call the Stanley controller if abs(CTE) > 5
    cte, distance_to_next_waypoint = NearestWayPointCTEandDistance(UTMp)

    if abs(cte) > 6.:
        steering_angle = stanley(float(cte), float(speed))
        takeover = True
    else:
        takeover = False
    ...
    #steering_angle = keyleftright #leftright + keyleftright
    # The driving model currently just outputs a constant throttle. Feel free to edit this.
    if throttle < 0:
       throttle = 0
    ### we can force the car to slow down using the up joystick movement.
    #throttle = keyupdown #updown + keyupdown

    print(steering_angle, throttle, takeover, cte)
...
class KomandaModel(object):
    ...
    def __init__(self, checkpoint_dir, metagraph_file):
        ...
        self.output_tensors = map(self.graph.get_tensor_by_name, ["output_steering:0", "output_throttle:0","controller_final_state_0:0", "controller_final_state_1:0", "controller_final_state_2:0"])
