...
def generateImagesFromPaths(data, batchSize, inputShape, outputShape, train = False):
    ...
    while 1:
        ...
        yield({'inputImg': returnArr,'inputSpeed': np.array(speedArr)},
              {'outputSteer': labels[:,0],'outputThr': labels[:,1]-labels[:,2], 'outputPos': np.array(vecArr)}, [10*weights,weights, 10*weights])
...
def main():
    ...
    hist = 1./np.array([val if val > len(dataNew)/40. else len(dataNew)/40. for val in hist])
    hist*=len(dataNew)/40.
    print(hist,edges, len(dataNew))
    ...
    visCallback = TensorBoard(log_dir = './logs/%d' %time.time())
    model = load_model('psyncModelBase.h5', custom_objects={'customLoss':customLoss})
    if LOADMODEL:
        endModel = load_model('psyncModelGPSRaw.h5', custom_objects={'customLoss':customLoss})
        ...
        endModel.save('psyncModelGPSRaw.h5')
        
        
    else:
        inpC = Input(shape=(imShape[0], imShape[1], imShape[2]), name='inputImg')
        xC = Convolution2D(24, 8, 8,border_mode='valid', subsample=(2,2), name='conv1', 
                           weights=model.get_layer('conv1').get_weights(), trainable = False)(inpC)
        xC = BatchNormalization()(xC)
        xC = Activation('elu')(xC)
        print(xC.get_shape())
        xC = Convolution2D(36, 5, 5, border_mode='valid',subsample=(2,2), name='conv2', 
                           weights=model.get_layer('conv2').get_weights(), trainable = False)(xC)
        xC = BatchNormalization()(xC)
        xC = Activation('elu')(xC)
        print(xC.get_shape())
        xC = Convolution2D(48, 5, 5, border_mode='valid',subsample=(2,2), name='conv3', 
                           weights=model.get_layer('conv3').get_weights(), trainable = False)(xC)
        xC = BatchNormalization()(xC)
        xC = Activation('elu')(xC)
        print(xC.get_shape())
        xC = Convolution2D(64, 5, 5, border_mode='valid', name='conv4', 
                           weights=model.get_layer('conv4').get_weights(), trainable = False)(xC)
        xC = BatchNormalization()(xC)
        xC = Activation('elu')(xC)
        print(xC.get_shape())
        xC = Convolution2D(64, 5, 5, border_mode='valid', name='conv5', 
                           weights=model.get_layer('conv5').get_weights(), trainable = False)(xC)
        xC = BatchNormalization()(xC)
        xC = Activation('elu')(xC)
        print(xC.get_shape())
        xOut = Flatten()(xC)
        ...
        xOutPos = Dense(2, activation='sigmoid',name='pos5')(xOutPos)
        ...
        endModel.fit_generator(trainGenerator, callbacks = [stopCallback, checkCallback,visCallback], 
                               nb_epoch=60, samples_per_epoch=epochBatchSize, 
                               max_q_size=24,
                               nb_worker=8, pickle_safe=True)
        endModel.load_weights('psyncModel.ckpt')
        endModel.save('psyncModelGPSRaw.h5')
