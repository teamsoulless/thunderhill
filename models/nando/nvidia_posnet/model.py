...
def NvidiaModel(dropout):
    ...
    # Images
    image = Input(shape=(HEIGHT, WIDTH, DEPTH), name='inputImg')
    ...
    model = Model(input=image, output=prediction)
    model.compile(optimizer=Adam(lr=1e-4), loss='mean_absolute_error', metric=['mse', 'mean_absolute_error'])
    return model
...
if __name__ == '__main__':
    ...
    df_train, df_val = getDataFromThunderhill(args.dataset, split=True, randomize=True)
    print('TRAIN:', len(df_train))
    print('VALIDATION:', len(df_val))
    print(df_train[['speed', 'throttle', 'brake', 'norm']].describe())
