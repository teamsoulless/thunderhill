...
import argparse
import os
from loader import generate_thunderhill_batches, genSim001, genSim002,genSim003, genSession5,genPolysync0,genPolysync2,genAll,generate_thunderhill_batches_time
from config import *
import time
...
def NvidiaRNNModel(learning_rate, dropout):
    input_model = Input(shape=(TIME, HEIGHT, WIDTH, DEPTH))
    x = TimeDistributed(Convolution2D(24, 5, 5, border_mode='valid', subsample=(2, 2), W_regularizer=l2(learning_rate)))(input_model)
    x = ELU()(x)
    x = TimeDistributed(Convolution2D(36, 5, 5, border_mode='valid', subsample=(2, 2), W_regularizer=l2(learning_rate)))(x)
    x = ELU()(x)
    x = TimeDistributed(Convolution2D(48, 5, 5, border_mode='valid', subsample=(2, 2), W_regularizer=l2(learning_rate)))(x)
    x = ELU()(x)
    x = TimeDistributed(Convolution2D(64, 3, 3, border_mode='valid', subsample=(1, 1), W_regularizer=l2(learning_rate)))(x)
    x = ELU()(x)
    x = TimeDistributed(Convolution2D(64, 3, 3, border_mode='valid', subsample=(1, 1), W_regularizer=l2(learning_rate)))(x)
    x = ELU()(x)
...
if __name__ == '__main__':
    ...
    model = NvidiaRNNModel(args.alpha, args.dropout)
    ...
    history = model.fit_generator(
        generate_thunderhill_batches_time(genSim003(args.dataset), args.batch),
        nb_epoch=args.epoch,
        samples_per_epoch=args.batch*50,
        validation_data=generate_thunderhill_batches_time(genSim001(args.dataset), args.batch),
